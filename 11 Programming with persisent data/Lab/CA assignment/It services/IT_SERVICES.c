...
//global struct variable for calucaltion purposes
struct user_info_and_problem global;

//implement main
int main()
{
	//implementing each structure to point to each other
	struct user_info_and_problem mark = {"Mark",
										"1212",
										"Router lights amber",
										"12:10",
										NULL};

	struct user_info_and_problem sean = {"Sean",
										"5656",
										"Desktop won't flicker",
										"12:06",
										&mark};

	struct user_info_and_problem jane = {"Jane",
										"4558",
										"Monitor broken",
										"12:05",
										&sean};
...
void person_input(struct user_info_and_problem *head, int *count)
{
	if ((*count) < MAX)
	{
		...
		struct user_info_and_problem *current = head;

		//int variables
		int count_for_characters;
        unsigned int i;
		...
		current->next =  (struct user_info_and_problem*)malloc(sizeof(global));
