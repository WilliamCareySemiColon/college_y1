...
void main()
{
	...
	//FILE variables
	FILE *fp = fopen("funded_eu.txt","r");
	FILE *fp2 = fopen("unfunded_eu.txt","r");
	FILE *fp3 = fopen("international.txt","r");
	FILE *fp_final = fopen("DT_265_postgratuate_student_list.txt","w");
	...
	else
	{
		...
		puts("\nGetting information from tbe first file");
		index = 0;

		//Getting input fromn the first file
		while ((fscanf(fp,"%S %S %S %S",eu_funded[index].firstname,eu_funded[index].surname,eu_funded[index].age,eu_funded[index].student_id)) != EOF)
		{
		...
		puts("\nGetting information from the second file");
		index = 0;

		//getting input from the second file
		while((fscanf(fp2,"%S %S %S %S",eu_nonfunded[index].firstname,eu_nonfunded[index].surname,eu_nonfunded[index].age,eu_nonfunded[index].student_id)) != EOF)
		{
			//making sure we know the status of the student when we write it into another file
		...
		puts("\nGeting infromation from the third file");
		index = 0;

		//getting input from the third file
		while ((fscanf(fp3,"%S %S %S %S",international[index].firstname,international[index].surname,international[index].age,international[index].student_id)) != EOF)
		{
			//making sure we know the status of the student when we write it into another file
			strcpy(eu_funded[index].type,"International");

			printf("%S %S %S %S ",international[index].firstname,international[index].surname,international[index].age,international[index].student_id);
			//Incrementing the postion
			index++;
		}

		puts("\nSorting out the first array");
		//sorting the first array
		merge_sort(0, (FUNDED_EU - 1), FUNDED_EU, eu_funded);

		puts("\nSorting out the second array");
		//sorting the second array
		merge_sort(0, (UNFUNDED_EU - 1),UNFUNDED_EU, eu_nonfunded);

		puts("\nSorting the third array");
		//sorting the third array
		insertion_sort(0, (STUDENT_ID - 1), STUDENT_ID, international);

		//Starting to copy the contents of the three arrays into one

		puts("\nCoping the contents of the arrays into the final");
		...
		{
			printf("Name: %s Surname: %s Age: %s Student Status: %s Student ID: %s\
