...
void main()
{
	...
	else
	{
		...
		while ((fscanf(fp,"%s %s %s %s",
                 eu_funded[index].firstname,eu_funded[index].surname,eu_funded[index].age,eu_funded[index].student_id)) != EOF
          && index != FUNDED_EU)
		{
		...
		while((fscanf(fp2,"%s %s %s %s",
                eu_nonfunded[index].firstname,eu_nonfunded[index].surname,
                eu_nonfunded[index].age,eu_nonfunded[index].student_id)) != EOF && index != UNFUNDED_EU)
		{
			//making sure we know the status of the student when we write it into another file
		...
		while ((fscanf(fp3,"%s %s %s %s",
                 international[index].firstname,international[index].surname,international[index].age,
                 international[index].student_id)) != EOF && index != STUDENT_ID)
		{
			//making sure we know the status of the student when we write it into another file
		...
		merge_sort(0, (TOTAL - 1), TOTAL, altogether);
